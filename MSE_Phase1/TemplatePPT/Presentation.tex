...
% --- YELLOW block: solid theme yellow, RGB(223,165,15) ---
\definecolor{coepyellow}{RGB}{223,165,15}
\newenvironment{yellowblock}[1]{%
  \setbeamercolor{block title}{bg=coepyellow, fg=black}%
  \setbeamercolor{block body}{bg=coepyellow, fg=black}%
  \begin{block}{#1}}%
  {\end{block}}
...
%%------------------------------------------------
\section[Literature]{Literature Survey}
...
\vspace{0.8em}
\begin{itemize}
  \item Evaluated on temporal and severity (out-of-distribution) splits of a
        synthetic supply-chain benchmark
\end{itemize}
\end{frame}
%---------------------------------------------
%%--------------------------------------------
\section[Objectives]{Research Objectives}
\begin{frame}
...
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.6cm}
                               ...
                               >{\raggedright\arraybackslash}X@{}}
\multicolumn{3}{@{}c@{}}{\textbf{Table 4: Concrete Future Plan}} \\
\toprule
\textbf{No.} & \textbf{Stage} & \textbf{Planned Work (one factor varied at a time)} \\
\midrule
1 & Benchmark and graph &
Synthetic supply-chain dataset with black-swan events; heterogeneous graph
construction \\ \rowgap

2 & Classical baseline &
GraphSAGE encoder trained and evaluated on temporal and severity splits \\ \rowgap

3 & QGNN baseline &
VQC head on the frozen GraphSAGE embedding; training stabilisation (patience,
output scaling, LayerNorm) \\ \rowgap

4 & Circuit ablations &
Qubit count (4, 6, 8) and circuit depth (1, 2, 3 layers) \\ \rowgap

5 & Representation audit &
Diagnostic classifiers on the frozen embedding to check its signal \\ \rowgap

6 & Head design &
Projection architecture and parameter initialization \\ \rowgap

7 & Ansatz and encoding &
Entanglement topology; encoding scale, type and data re-uploading \\ \rowgap

8 & Final evaluation &
Classical vs.\ QGNN over multiple seeds: ranking, calibration and stability
metrics \\
\bottomrule
\end{tabularx}
\end{frame}
...
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.6cm}
                               ...
                               >{\raggedright\arraybackslash}X@{}}
\multicolumn{3}{@{}c@{}}{\textbf{Table 5: Expected Outcomes}} \\
\toprule
\textbf{No.} & \textbf{Expected Outcome} & \textbf{How it will be assessed} \\
\midrule
1 & Working classical GraphSAGE baseline and hybrid QGNN for supplier
    disruption-risk prediction &
Both implemented on one shared data and training pipeline \\ \rowgap

2 & A controlled comparison of the two models &
Same data, splits, metrics and multiple seeds; PR-AUC as primary metric \\ \rowgap

3 & Understanding of how each model handles normal vs.\ severe conditions &
Temporal split and severity (out-of-distribution) split, reported separately \\ \rowgap

4 & A map of which design factors influence QGNN behavior &
One factor varied at a time: qubits, depth, ansatz, encoding, initialization \\ \rowgap

5 & Insight into how much risk signal the frozen GraphSAGE embedding carries &
Diagnostic classifiers on the frozen embedding, compared with both model heads \\ \rowgap

6 & Clearly scoped conclusions &
Stated for a synthetic benchmark on an ideal simulator, not as a general
